"""Run the Nepali crash feature, clustering, and heatmap pipeline."""
...
STAGES = [
    (1, "pipeline/01_ingest.py", "Nepali crash ingestion"),
    (2, "pipeline/02_features.py", "Feature extraction and clustering"),
    (3, "pipeline/05_kde.py", "KDE heatmap"),
]
...
def main() -> None:
    ...
    for pattern in ("*.parquet", "*.json"):
        for f in sorted((BASE / "data").glob(pattern)):
